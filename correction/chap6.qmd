...
::: {#exr-6-4 name="Ozone"}
1.  Les résultats sont dans l'ordre : 45, 24.663, 5.886, 1.567, 0.581, -4.547.

2.  La statistique de test de nullité du paramètre se trouve dans la colonne `t`, nous conservons $\Hz$ pour les
paramètres associés à `T12` et `T15`, et la rejetons pour les autres.

3.  La statistique de test de nullité simultanée des paramètres autres que la constante vaut 24.463. Nous rejetons $\Hz$.
...
5.  Nous sommes en présence de modèles emboîtés, nous pouvons appliquer la formule adaptée (voir l'exercice précédent) :
    ...
    \end{eqnarray*}
    La quantile vaut 3.20, nous rejetons donc $\Hz$ et privilégions le plus grand modèle.
:::

::: {#exr-6-5 name="Équivalence du test T et du test F"}
...
::: {#exr-6-6 name="Équivalence du test F et du test de VM"}
...
::: {#exr-6-7 name="Test de Fisher pour une hypothèse linéaire quelconque"}
...
::: {#exr-6-8 name="Généralisation de la régression ridge"}
...
::: {#exr-6-9 name="IC pour la régression ridge"}
1.  Loi de $\hat \beta$ : $N(\beta, \sigma^{2}(X'X)^{-1})$ grâce au modèle et à $\HH_3$.

2.  Loi de $\hat \beta_{\mathrm{ridge}}(\tilde\kappa) $. Comme $\hat \beta_{\mathrm{ridge}}(\tilde\kappa)= (X'X-\tilde\kappa I)^{-1}X'Y$ avec $A=(X'X-\tilde\kappa I)^{-1}X'$ qui est une matrice fixe. Avec $\HH_{3}$ et le modèle de régression multiple on a que $Y\sim N(X\beta, \sigma^{2}I)$.
